#!/usr/bin/env python3
"""
Standard COCO Evaluation for SAM Segmentation with Pruning Support.

This extends coco_evaluation.py to support pruned SAM models.
"""
...
class StandardCOCOEvaluatorWithPruning:
    """COCO Evaluation pipeline for SAM with pruning support."""
    ...
    def __init__(
        ...
        print(f"Loading COCO ground truth: {coco_annotation_file}")
        self.coco_gt = COCO(coco_annotation_file)
    ...
    def evaluate(self, max_images: Optional[int] = None, batch_size: int = 1) -> Dict:
        ...
        if self.save_predictions:
            ...
            pred_file = os.path.join(self.output_dir, 'coco_predictions.json')
        ...
        # For mask evaluation, remove bbox
        eval_predictions = copy.deepcopy(all_predictions)
        for c in eval_predictions:
            c.pop("bbox", None)
        
        coco_dt = self.coco_gt.loadRes(eval_predictions)
        coco_eval = COCOeval(self.coco_gt, coco_dt, 'segm')
        coco_eval.params.maxDets = [1, 10, 100]
        
        eval_img_ids = list(set(p['image_id'] for p in all_predictions))
...
def main():
    parser = argparse.ArgumentParser(
        description='Standard COCO Evaluation for SAM with Pruning Support'
    )
    parser.add_argument('--sam-checkpoint', type=str, required=True)
    parser.add_argument('--model-type', type=str, default='vit_l',
                        choices=['vit_h', 'vit_l', 'vit_b'])
    parser.add_argument('--coco-annotation', type=str, required=True)
    ...
    parser.add_argument('--output-file', type=str, default='coco_results.json')
    ...
    evaluator = StandardCOCOEvaluatorWithPruning(
    ...
    print("\n" + "=" * 60)
    print("Starting COCO Evaluation with Pruning")
    print("=" * 60)
